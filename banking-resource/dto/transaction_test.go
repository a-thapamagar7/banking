...
func Test_should_return_error_when_transaction_type_is_not_desposit_or_withdrawl(t *testing.T) {
	// Arrange
...
func Test_should_return_error_when_amount_is_less_than_zero(t *testing.T) {
	...
	if appError.Message != "Amount cannot be less than zero" {
		t.Error("Invalid message while validating amonut")
	}
	if appError.Code != http.StatusUnprocessableEntity {
		t.Error("Invalid code while validating amonut")
	}
}
